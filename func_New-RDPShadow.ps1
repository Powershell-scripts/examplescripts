...
#>
    ...
    $Error = "Не должно быть больше двух ПК в списке. Посмотри и вбей нужный." + "`n`r" + "Список ниже: "
    $PCNames = Get-ADComputer -Filter "name -like '$PCName'"
    if ($PCNames.name.count -gt 1) {
        $ComputerNames = $PCNames.Name
        $Error + "`n`r" + "$ComputerNames"
        break
        }
...
.EXAMPLE
 ...
 Осуществит подключение к кассе на павелецкой 

.EXAMPLE
 New-RDPShadow -PCName *pvl-cash* -IDUser 1
 Осуществит подключение к кассе на павелецкой c заданным вручную параметром IDSession
